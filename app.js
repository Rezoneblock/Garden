...
window.onload = function() { 
  document.querySelector('#randomSeed').addEventListener('click', function(event) { // Выбрать рандомное кол-во деревьев и яблок
    document.querySelector('#treesInitInput').value = Math.round(Math.random() * 50 + 1) // Рандом до 50, вручную можно указать больше (до 100)
    document.querySelector('#applesInitInput').value = Math.round(Math.random() * 50 + 1) // Рандом до 50, вручную можно указать больше (до 100)
  })
